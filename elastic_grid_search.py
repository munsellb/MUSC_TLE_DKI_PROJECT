...
x_input = np.vstack((masked_matrices.ix[mode].cons,masked_matrices.ix[mode].pats))
print 'training data for modality {} includes {} examples and {} features'.format(mode.upper(),*x_input.shape)
# creating y output vector (this is ugly fix this) (yyy must have shape of (trn_exmpl,-))
y_len = map(len,masked_matrices.ix[mode])
